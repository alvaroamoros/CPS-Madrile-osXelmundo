...
display.brewer.all()
scale_fill_brewer(palette = "YlBlues")
...
cnt <- plyr::count(madrileños$antes_pais)
madrileños$antes_pais <- factor(madrileños$antes_pais,
                                levels = cnt$x[order(cnt$freq, decreasing = TRUE)])

antes_pais_g <- madrileños %>%
  filter(!is.na(antes_pais)) %>%
  ggplot(aes(antes_pais)) +
  scale_fill_brewer(palette="Blues", aesthetics = "fill", labels = c("No", "De turismo", "Vivido anteriormente", "Motivos laborales",
                                                                     "Visitando familiares", "Erasmus", "Otro", "Otra beca") ) +
  geom_bar(aes(y = (..count..)/sum(..count..), fill=factor(..x..)), stat= "count") +
  geom_text(aes(label = scales::percent((..count..)/sum(..count..), accuracy = 1),
                y= ((..count..)/sum(..count..))), stat="count",
            hjust = 0.5, size = 3, vjust= -1,
...
viajes_madrid_g <- madrileños %>%
  ...
  scale_fill_brewer(palette="Blues", aesthetics = "fill", labels = c("Nunca", "Menos de una vez al año", "Una vez al año",
...
viajes_madrid_g +guides(fill=guide_legend(nrow=3,byrow=TRUE, label.hjust = -0.1)) + ggsave("viajes_madrid_g.png")
...
frecuencia_gente_mad_g <- madrileños %>%
  ...
  scale_fill_brewer(palette="Blues", aesthetics = "fill", labels = c("Nunca", "Menos de una vez al mes", "Mensualmente",
...
registrado_consulado_g <- madrileños %>%
  ...
  scale_fill_brewer(palette="Blues", aesthetics = "fill", labels = c("Sí, como residente", "Sí, como no residente", "No")) +
  geom_bar(aes(y = (..count..)/sum(..count..), fill=factor(..x..)), stat= "count") +
  geom_text(aes(label = scales::percent((..count..)/sum(..count..), accuracy = 1),
                y= ((..count..)/sum(..count..))), stat="count",
            hjust = 0.5, size = 3, vjust= -0.5,
...
motivo_registro_g <- madrileños %>%
  ...
  geom_bar(aes(y = (..count..)/sum(..count..), fill=factor(..x..)), stat= "count") +
  scale_fill_manual(values = mycolors, aesthetics = "fill", labels = c("Catástrofes / emergencias", "Renovar mi documentación", "Votar en España","Exención IVA", "Apertura cuentas bancarias",
                                                                       "Cert. emigrante retornado", "Obligación/responsabilidad", "Contacto con España /
  localizable COVID-19", "Nacionalización hijos", "Franquicia  aduanera", "Casarse", "Otros") ) +
  geom_text(aes(label = scales::percent((..count..)/sum(..count..), accuracy = 1),
                y= ((..count..)/sum(..count..))), stat="count",
            hjust = 0.5, size = 3, vjust= -0.5,
            inherit.aes = TRUE) +
...
motivo_registro_g + guides(fill=guide_legend(nrow=4,byrow=TRUE, label.hjust = -0.09 )) + ggsave("motivo_registro_g.png") 
...
motivo_no_registro_g <- madrileños %>%
  ...
  geom_bar(aes(y = (..count..)/sum(..count..), fill=factor(..x..)), stat= "count") +
  scale_fill_brewer(palette="Blues", aesthetics = "fill", labels = c("No ser baja en 
  el patrón de España.", "Trámites complicados.", "No me interesa.", "No sabía de la posibilidad", "Falta de tiempo", "Crísis COVID19 lo 
  ha impedido", "NS/NC", "No sabía como", "No quiero que la admin. sepa
  mi residencia") ) +
  geom_text(aes(label = scales::percent((..count..)/sum(..count..), accuracy = 1),
                y= ((..count..)/sum(..count..))), stat="count",
            hjust = 0.5, size = 3, vjust= -0.5,
...
motivo_no_registro_g+ guides(fill=guide_legend(nrow=4,byrow=TRUE, label.hjust = -0.1)) +  ggsave("motivo_no_registro.png")
...
prob_trabajo_g <- madrileños %>%
  ...
  scale_fill_brewer(palette="Blues", aesthetics = "fill", labels = c("Muy probable", "Bastante probable", "Poco probable", "Nada probable") ) +
  geom_bar(aes(y = (..count..)/sum(..count..), fill=factor(..x..)), stat= "count") +
  geom_text(aes(label = scales::percent((..count..)/sum(..count..), accuracy = 1),
                y= ((..count..)/sum(..count..))), stat="count",
            hjust = 0.5, size = 3, vjust= -0.5,
...
estado_civil_g <- madrileños %>%
  ...
  filter(!estado_civil %in% c("Ns/Nc")) %>%
  ggplot(aes(estado_civil)) +
  geom_bar(aes(y = (..count..)/sum(..count..), fill=factor(..x..)), stat= "count") +
  scale_fill_brewer(palette="Blues", aesthetics = "fill", labels = c("Casado/a", "Soltero/a", "Divorciado", "Separado/a", "Viudo") ) +
  geom_text(aes(label = scales::percent((..count..)/sum(..count..), accuracy = 1),
                y= ((..count..)/sum(..count..))), stat="count",
            hjust = 0.5, size = 3, vjust= -0.5,
            inherit.aes = TRUE) +
...
vive_solo_g <- madrileños %>%
  ...
  geom_bar(aes(y = (..count..)/sum(..count..), fill=factor(..x..)), stat= "count") +
  scale_fill_brewer(palette="Blues", aesthetics = "fill", labels = c("Con mi Pareja", "Solo/a", "Compañeros/as de piso", "Con mi pareja y algún
  familiar", "Con amigos", "Con algún familiar") ) +
  geom_text(aes(label = scales::percent((..count..)/sum(..count..), accuracy = 1),
                y= ((..count..)/sum(..count..))), stat="count",
            hjust = 0.5, size = 3, vjust= -0.5,
            inherit.aes = TRUE) +
...
hijos_g <- madrileños %>%
  ...
  ggplot(aes(hijos)) +
  geom_bar(aes(y = (..count..)/sum(..count..), fill=factor(..x..)), stat= "count") +
  scale_fill_brewer(palette="Blues", aesthetics = "fill", labels = c("Sí, vive conmigo", "Sí, vive en el mismo país", "Sí, vive en España", "Sí, vive en otro país", "No") ) +
  geom_text(aes(label = scales::percent((..count..)/sum(..count..), accuracy = 1),
                y= ((..count..)/sum(..count..))), stat="count",
            hjust = 0.5, size = 3, vjust= -0.5,
            inherit.aes = TRUE) +
...
hijos_g +guides(fill=guide_legend(nrow=3,byrow=TRUE, label.hjust = -0.1))+ ggsave("hijos_g.png")
...
tamaño_municipio_g <- madrileños %>%
  ...
  geom_bar(aes(y = (..count..)/sum(..count..), fill=factor(..x..)), stat= "count") +
  scale_fill_brewer(palette="Blues", aesthetics = "fill", labels = c("Ciudad (más de 500.001)", "Ciudad (100.001 a 500.000)", "Ciudad (20.001 a 100.000)", "Ciudad (1.001 a 20.000)", "Aldela/pueblo") ) +
  geom_text(aes(label = scales::percent((..count..)/sum(..count..), accuracy = 1),
                y= ((..count..)/sum(..count..))), stat="count",
            hjust = 0.5, size = 3, vjust= -0.5,
            inherit.aes = TRUE) +
  scale_y_continuous(labels=scales::percent_format(accuracy = 5L)) +  theme(legend.position = "none") +
  theme_hc() +       
  theme(legend.position = "bottom",
        legend.title = element_blank(),
        legend.text = element_text(size = 8),
...
cnt <- plyr::count(madrileños$E8)
madrileños$E8 <- factor(madrileños$E8,
                              levels = cnt$x[order(cnt$freq, decreasing = TRUE)])

estudios_g <- madrileños %>%
  filter(!is.na(E8)) %>%
  ggplot(aes(E8)) +
  scale_fill_brewer(palette="Blues", aesthetics = "fill", labels = c("Postgrado", "Grado", "Formación profesional", "Doctorado", "Secundaria", "Primaria")) +
  geom_bar(aes(y = (..count..)/sum(..count..), fill=factor(..x..)), stat= "count") +
  geom_text(aes(label = scales::percent((..count..)/sum(..count..), accuracy = 1),
                y= ((..count..)/sum(..count..))), stat="count",
            hjust = 0.5, size = 3, vjust= -0.5,
...
tipo_contrato_g <- madrileños %>%
  ...
  geom_bar(aes(y = (..count..)/sum(..count..), fill=factor(..x..)), stat= "count") +
  scale_fill_brewer(palette="Blues", aesthetics = "fill", labels = c("Indefinido", "Temporal", "No tengo")) +
  geom_text(aes(label = scales::percent((..count..)/sum(..count..), accuracy = 1),
                y= ((..count..)/sum(..count..))), stat="count",
            hjust = 0.5, size = 3, vjust= -0.5,
            inherit.aes = TRUE) +
...
dedicacion_g <- madrileños %>%
  ...
  geom_bar(aes(y = (..count..)/sum(..count..), fill=factor(..x..)), stat= "count") +
  scale_fill_brewer(palette="Blues", aesthetics = "fill", labels = c("Jornada completa", "Jornada parcial", "Por horas")) +
  geom_text(aes(label = scales::percent((..count..)/sum(..count..), accuracy = 1),
                y= ((..count..)/sum(..count..))), stat="count",
            hjust = 0.5, size = 3, vjust= -0.5,
            inherit.aes = TRUE) +
...
dedicacion_g +  guides(fill=guide_legend(nrow=2,byrow=TRUE, label.hjust = -0.1))  +ggsave("dedicacion_g.png")
